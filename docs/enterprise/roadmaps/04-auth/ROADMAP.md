...
- [ ] **AU-001**: Adicionar novos roles ao enum `EmployeeRole`
  ```prisma
  enum EmployeeRole {
    ADMIN
    MANAGER
...
- [ ] **AU-002**: Documentar descrição de cada role
...
- [ ] **AU-003**: Criar `permissions/enterprise.ts`
  ```typescript
...
- [ ] **AU-008**: Implementar vinculação de funcionário a contratos
  ...
  function useAccessibleContracts() {
    ...
    return useQuery({
      ...
      queryFn: () => invoke('list_accessible_contracts', { 
        employeeId: user?.id 
      }),
    });
  }
