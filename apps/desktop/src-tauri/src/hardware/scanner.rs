...
async fn handle_scanner_connection(
    ...
    write.send(Message::Text(serde_json::to_string(&connected).unwrap()))
        .await
        .map_err(|e| HardwareError::CommunicationError(e.to_string()))?;
                ...
                if let Ok(scanner_msg) = serde_json::from_str::<ScannerMessage>(text_str) {
                    match scanner_msg {
                        ...
                        ScannerMessage::Ping => {
                            ...
                            write.send(Message::Text(serde_json::to_string(&pong).unwrap()))
                                .await
                                .ok();
                        }
                        
                        ScannerMessage::Register { device_id: id, device_name } => {
