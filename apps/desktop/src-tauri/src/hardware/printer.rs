...
impl ThermalPrinter {
    ...
    pub fn qrcode(&mut self, data: &str) -> &mut Self {
        ...
        self.buffer.extend_from_slice(&[escpos::GS, b'(', b'k', pl, ph, 49, 80, 48]);
    ...
    pub fn test_print(&mut self) -> HardwareResult<()> {
        ...
        self.style(TextStyle { bold: true, double_height: true, ..Default::default() });
        ...
        self.feed(1);
        self.align(TextAlign::Center);
        self.line(&format!("Data/Hora: {}", chrono::Local::now().format("%d/%m/%Y %H:%M:%S")));
    ...
    pub fn print_receipt(&mut self, receipt: &Receipt) -> &mut Self {
        ...
        for item in &receipt.items {
            ...
            if item.quantity == 1.0 {
                ...
                self.line(&format!("{}{:>width$}", name, price_str, width = spaces + price_str.len()));
            } else {
...
mod tests {
    ...
    fn test_receipt_builder() {
        let config = PrinterConfig { paper_width: 48, ..Default::default() };
        let mut printer = ThermalPrinter::new(config);
