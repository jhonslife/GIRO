...
export const commands = {
   ...
   */
  async printAttendantOrder(order: AttendantOrderReceipt): Promise<Result<null, AppError>> {
    try {
      return { status: 'ok', data: await TAURI_INVOKE('print_attendant_order', { order }) };
    } catch (e) {
      if (e instanceof Error) throw e;
      else return { status: 'error', error: e as any };
    }
...
export type AppError =
  ...
  | { StockNegative: { current: number; new: number } }
  | 'PriceInversion'
  | 'PermissionDenied'
  | { Warning: string }
  | { Hardware: string }
...
export type LicenseInfo = {
  ...
  can_offline: boolean | null;
  has_admin: boolean | null;
  admin_user: AdminUserSyncData | null;
  /**
...
export type SaleWithDetails = {
  ...
  createdAt: string;
  updatedAt: string;
} & { employeeName: string | null; items: SaleItem[]; payments: SalePayment[]; itemsCount: number };
 ...
 */
export type AttendantOrderReceipt = {
  companyName: string;
  companyAddress: string;
  companyPhone: string | null;
  orderNumber: string;
