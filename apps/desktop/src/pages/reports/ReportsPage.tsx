            ...
            <div className="space-y-6">
              {topProducts?.slice(0, 5).map((item, i) => (
                <div key={item.product.id} className="flex items-center gap-4">
                  <div className="flex h-9 w-9 items-center justify-center rounded-full bg-muted/50 text-sm font-bold">
                    {i + 1}
                  </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {REPORTS.map((report) => (
          <Link
            key={report.id}
            to={report.href}
            className="group relative overflow-hidden rounded-xl border-none p-6 shadow-sm bg-card/50 hover:bg-card/80 transition-all hover:scale-[1.02]"
          >
